...
Gui Add, ListView, hWndhLV vList gLVHandler x0 y30 h%LVH% w%WindowW% +LV0x14000 AltSubmit, Filename|Path|PID

IniRead Columns, %IniFile%, Position, Columns, 177|499|53
...
LoadList() {
    ...
    For Process in winmgmts.ExecQuery(StrQuery) {
        ...
        If RegExMatch(CommandLine, "(.:[^:]+\\([^\x22]+))", m) {
            ...
            LV_Add("Icon" . IconIndex, m2, m1, PID)
        }
    }
}
...
GuiContextMenu:
    If (A_GuiControl == "List" && Row := LV_GetNext()) {
        ...
        ShellContextMenu(Filename, hMainWnd)
    }
Return
...
GetScriptState(PID, State) {
    ...
    If (WinExist("ahk_id " . hWnd)) {
        ...
        Checked := DllCall("GetMenuState", "Ptr", hMenu, "UInt", Command, "UInt", 0) ; By command
        Return Checked == 0x8
    } Else {
        Return 0
    }
...
UpdateToolbar() {
    ...
    SendMessage 0x402, 10004, % GetScriptState(PID, "S"),, ahk_id %hToolbar% ; TB_CHECKBUTTON
    SendMessage 0x402, 10005, % GetScriptState(PID, "P"),, ahk_id %hToolbar% ; Bug: Pause is set on WM_ENTERMENULOOP
}
