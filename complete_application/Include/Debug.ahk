...
DebugRun(AltRun := False) {
    If (g_DbgStatus == 0) {
        ...
        SplitPath AhkScript,, WorkingDir

        If (g_CaptureStdErr) {
            AhkRunGetStdErr(n, AhkPath, AhkScript, g_Parameters, WorkingDir, "/Debug=127.0.0.1:" . g_DbgPort)

        } Else {
            RunWait "%AhkPath%" /debug=127.0.0.1:%g_DbgPort% "%AhkScript%" %g_Parameters%, %WorkingDir%, UseErrorLevel
            If (ErrorLevel) {
                GoSub DebugError
            }
        }
