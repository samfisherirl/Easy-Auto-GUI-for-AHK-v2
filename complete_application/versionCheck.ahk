UpdateCheck() {
    ...
    zipperPath := A_ScriptDir "\complete_application\7za.exe"
    settings_path := A_ScriptDir "\complete_application\settings.json"
    updater := A_ScriptDir "\complete_application\updater.exe"
    ...
    if (version_info["version"] != git.Version) {
        ...
        if count < 4 {
            ...
            if answer = "Yes" {
                ...
                F.Write(serialized)
                F.Close()
                FileMove(A_ScriptDir "\complete_application\updater.exe", A_ScriptDir "\updater.exe", 1)
                RunWait(A_ScriptDir "\updater.exe")
                FileMove(A_ScriptDir "\updater.exe", A_ScriptDir "\complete_application\updater.exe", 1)
