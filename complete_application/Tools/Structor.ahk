...
Gui Add, Button, vBtnParse gGetOffsets x10 y224 w84 h24 +Disabled, &Parse
...
Gui Font, s9, Segoe UI
Gui Add, ListView, hWndhOffsetList vLV x11 y286 w603 h180 +LV0x114004, Member|Data Type|32-bit Offset|64-bit Offset
LV_ModifyCol(1, 150)
LV_ModifyCol(2, 150)
LV_ModifyCol(3, "80 Integer")
LV_ModifyCol(4, "80 Integer")

Gui Add, Button, vBtnCopy gGenerateCode x9 y476 w84 h24, &Copy
Gui Add, CheckBox, vChkNumGet x110 y476 w96 h23 +Checked%ChkNumGet%, NumGet
Gui Add, CheckBox, vChkNumPut x208 y476 w96 h23 +Checked%ChkNumPut%, NumPut
...
DllCall("UxTheme.dll\SetWindowTheme", "Ptr", hOffsetList, "WStr", "Explorer", "Ptr", 0)
...
; Generate Compile32.bat and Compile64.bat
CreateBatchFile(xNN) {
    ...
    SplitPath CompilerPath%xNN%, Filename
    If (Filename = "cl.exe") {
        Bat .= """" . CompilerPath%xNN% . """ Offsets.c -FeOffsets" . xNN . ".exe"
    } Else if (Filename = "gcc.exe") {
        Bat .= """" . CompilerPath%xNN% . """ Offsets.c -o Offsets" . xNN . ".exe"
    }
...
MsgBox 0x0, Help, %AppName% requires a C compiler. Supported: Visual Studio and MinGW.`n`n♦ Visual Studio`, Windows SDK`n`nCompiler Path: path to CL.EXE. Example:`nC:\Program Files (x86)\Microsoft Visual Studio 11.0\VC\bin\cl.exe`n`nBatch File: path to VCVARS32.bat or VCVARS64.BAT. Example:`nC:\Program Files (x86)\Microsoft Visual Studio 11.0\VC\bin\vcvars32.bat`n`n♦ MinGW`, TDM-GCC`n`nCompiler Path: path to GCC.EXE. Example:`nC:\Dev-Cpp\bin\gcc.exe`n`nBatch File: only needed if the gcc.exe directory is not in the PATH.`n`nEnable "Pause Command Prompt" to see compiler error messages.
Return
